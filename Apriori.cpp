...
// Generic Unordered Map Template[Idea from cplusplus.com ]:
...
//Template for Generic Unordered_map
template < typename SEQUENCE, typename T >
using Generic_unordered_map = std::unordered_map< SEQUENCE, T, seq_hash<SEQUENCE> > ;

//Considering txt file as Database
std::ifstream DB;

//candidates generated from Lk
void GetCandidates(vector<vector<long> >& TempItemSet, vector<vector<long> > KSize_Freq_ItmSet, Generic_unordered_map<vector<long>,bool> KSize_Freq_HashMap, int k);

//increment the count of all candidates in Ck+1 that are contained in transaction
void IncrementHashMapCounter(Generic_unordered_map<vector<long>,long>& HashMap, Generic_unordered_map<vector<long>,bool> CandidateItem_HashMap,  const vector<vector<long> >& CandidateItemSet, int k, unordered_map<long,bool>& UnwantedTransactions);
...
void Association_Rule_Gen(vector<pair<pair<vector<long>,vector<long>>,pair<float,float>>> & Assoc_Rules, const Generic_unordered_map<vector<long>, long>& FreqItemSet);
...
void OutputFrequentItemSet(const Generic_unordered_map<vector<long>, long>&  FreqItemSet);

//Outputting number of frequent itemsets of different sizes and the number of strong rules
void OutputResultCount(const Generic_unordered_map<vector<long>, long>&  FreqItemSet, const vector<pair<pair<vector<long>,vector<long>>,pair<float,float>>>& Strong_Assoc_Rules, double elapsed_secs);
...
int main(int argc, const char * argv[]) {
    ...
    Generic_unordered_map<vector<long>, long> FreqItemSet;
    ...
    Generic_unordered_map<vector<long>, long> HashMap; //To Fast Access the Count of Frequent Items
    ...
    if (DB.is_open())
    {
        ...
        while (getline(DB, Transaction))
        {
            ...
            for(int tokenIndex=0; tokenIndex<tokens.size(); tokenIndex++)
            {
                ...
                Generic_unordered_map<vector<long>, long>::const_iterator SearchKey = HashMap.find (Item);
    ...
    Generic_unordered_map<vector<long>, bool> KSize_Freq_HashMap;
    ...
    Generic_unordered_map<vector<long>, bool> Candidate_HashMap;
...
//candidates generated from Lk
void GetCandidates(vector<vector<long> >& CandidateItemSet, vector<vector<long> > KSize_Freq_ItmSet,Generic_unordered_map<vector<long>,bool> KSize_Freq_HashMap, int k)
{
        ...
        for(it2 = it1+1; it2 < FreqSetSize; ++it2)
        {
            ...
            if(Is_Diff_One)
            {
                ...
                for(vector<long>::iterator it3 = candidate_elmnt.begin(); it3 != candidate_elmnt.end(); ++it3)
                {
                    ...
                    Generic_unordered_map<vector<long>, bool>::const_iterator SearchKey = KSize_Freq_HashMap.find (temp_elmnt);
...
//increment the count of all candidates in Ck+1 that are contained in transaction
void IncrementHashMapCounter(Generic_unordered_map<vector<long>,long>& HashMap, Generic_unordered_map<vector<long>,bool> Candidate_HashMap, const vector<vector<long> >& CandidateItemSet, int k, unordered_map<long,bool>& UnwantedTransactions)
{
    DB.clear();
    DB.seekg (0, DB.beg);
            ...
            if(Itemsize>=k)
            {
                ...
                if(Cnt_Subset<=CandidateSize/50)//Checking All of the subsets
                {
                    ...
                    for(vector<vector<long> >::iterator it = TransSubsets.begin(); it != TransSubsets.end(); ++it)
                    {
                        ...
                        TempSet=(*it);
                        Generic_unordered_map<vector<long>, bool>::const_iterator SearchKey1 = Candidate_HashMap.find (TempSet);
                        if ( SearchKey1 != Candidate_HashMap.end() )
                        {
                            Contributed=true;
                            
                            Generic_unordered_map<vector<long>, long>::const_iterator SearchKey2 = HashMap.find (TempSet);
                ...
                else
                {
                    ...
                    for (auto &Candid_Item : CandidateItemSet)
                    {
                        ...
                        if(Caindid_Existed)
                        {
                            ...
                            //So it should
                            Generic_unordered_map<vector<long>, long>::const_iterator SearchKey2 = HashMap.find (Candid_Item);
                            if ( SearchKey2 == HashMap.end() )
                            {
                                //If the Item Does not exist in HashMap, Then Insert it
